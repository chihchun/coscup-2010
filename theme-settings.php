...
function coscup2010_settings($saved_settings, $subtheme_defaults = array()) {

  // Get the default values from the .info file.
  $defaults = coscup2010_theme_get_default_settings('coscup2010');
  ...
  $form['themedev']['coscup2010_rebuild_registry'] = array(
    '#type'          => 'checkbox',
    '#title'         => t('Rebuild theme registry on every page.'),
    '#default_value' => $settings['coscup2010_rebuild_registry'],
    '#description'   => t('During theme development, it can be very useful to continuously <a href="!link">rebuild the theme registry</a>. WARNING: this is a huge performance penalty and must be turned off on production websites.', array('!link' => 'http://drupal.org/node/173880#theme-registry')),
    '#prefix'        => '<div id="div-coscup2010-registry"><strong>' . t('Theme registry:') . '</strong>',
    '#suffix'        => '</div>',
  );
...
function _coscup2010_theme(&$existing, $type, $theme, $path) {
  ...
  coscup2010_theme_get_default_settings($theme);

  // If we are auto-rebuilding the theme registry, warn about feature.
  if (theme_get_setting('coscup2010_rebuild_registry')) {
...
function coscup2010_theme_get_default_settings($theme) {
  $themes = list_themes();
