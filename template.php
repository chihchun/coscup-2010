...
function coscup2010_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = coscup2010_id_safe('block-' . $vars['block']->module);
    $classes[] = coscup2010_id_safe('block-' . $vars['block']->region);
    $classes[] = coscup2010_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('coscup2010_block_editing') && user_access('administer blocks')) {
...
function coscup2010_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = coscup2010_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function coscup2010_id_safe($string) {
